...
class MPTranslate extends CApplicationComponent{
    ...
    static function missingTranslation($event) {
    	return self::translator()->missingTranslationHandler($event);
    }
     ...
     */
    function missingTranslationHandler($event){
    	if($event !== null && (substr($event->language, 0, 2) !== substr(Yii::app()->sourceLanguage, 0, 2) || Yii::app()->getMessages()->forceTranslation)) {
	        ...
	        if($model->id) {
	        	...
	        	if($messageModel !== null) {
		            ...
		            if($this->autoTranslate){//&& key_exists($event->language,$this->getGoogleAcceptedLanguages($event->language))
    ...
    function translateLink($label='Translate',$type='link'){
        $form=CHtml::form(Yii::app()->getController()->createUrl('translate/index'));
    ...
    function translateDialogLink($label='Translate',$title=null,$type='link'){
        return $this->ajaxDialog($label,'translate/index',$title,$type,array('data'=>array(self::ID.'-missing'=>self::$messages)));
    }
    ...
    function editLink($label='Edit translations',$type='link'){
        $url=Yii::app()->getController()->createUrl('translate/admin');
        if($type==='button')
    ...
    function missingLink($label='Missing translations',$type='link'){
        $url=Yii::app()->getController()->createUrl('translate/missing');
        if($type==='button')
            return CHtml::button($label,$url);
        else
            return CHtml::link($label,$url);
    ...
    function dropdown() {
        Yii::app()->getClientScript()->registerScript(self::ID.'-dropdown','
           $("#'.self::ID.'").change(function(){
                $.post(
                    "'.Yii::app()->getController()->createUrl("translate/set").'",
                    {"'.self::ID.'":$(this).val()},
